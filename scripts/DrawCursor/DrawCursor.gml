...
if (global.spellData[sp, spellC.Orientation] == 0 && global.spellData[sp, spellC.FlipOrientation] == 1) {
	var rot   = 0;
	var rxoff = 0;
	var ryoff = 0;
}
...
else if (global.spellData[sp, spellC.Orientation] == 90 && global.spellData[sp, spellC.FlipOrientation] == 1) {
...
else if (global.spellData[sp, spellC.Orientation] == 180 && global.spellData[sp, spellC.FlipOrientation] == 1) {
	var rot   = global.spellData[sp, spellC.Orientation];
...
else if (global.spellData[sp, spellC.Orientation] == 270 && global.spellData[sp, spellC.FlipOrientation] == 1) {
	var rot   = global.spellData[sp, spellC.Orientation];
	var rxoff = (cursorWidth * oPuzzleBoard.gridPadding) + 2;
	var ryoff = 0;
}
...
// Spell rotated 0 and flip -1
else if (global.spellData[sp, spellC.Orientation] == 0 && global.spellData[sp, spellC.FlipOrientation] == -1) {
	var rot   = 0;
	var rxoff = (cursorWidth * oPuzzleBoard.gridPadding) + 2;
	var ryoff = 0;
}
...
if (!oCombatManager.combatMenu.inSpellBook && !oPuzzleBoard.inCombatMenu && oCombatManager.combatMenu.attacking) {	
	draw_sprite_ext(
		cursorSprite, 0,
		((global.gridIndexX + cursorWidth  - (1 * cursorWidth))  * oPuzzleBoard.gridPadding) + xo + rxoff,
